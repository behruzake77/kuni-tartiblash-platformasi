    ...
    <Dialog open={addOpen} onOpenChange={setAddOpen} title="Rejaga vazifa qo‘shish" description={`${selected} kuni uchun yangi vazifa yarating.`}>
      <form onSubmit={addForSelectedDay} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div><label className="mb-1.5 block text-sm font-medium text-text-primary" htmlFor="calendar-time">Vaqt</label><Input id="calendar-time" type="time" value={time} onChange={(event) => setTime(event.target.value)} /></div>
        </div>
